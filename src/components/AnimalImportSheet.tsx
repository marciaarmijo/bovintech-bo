...
interface Props {
  open: boolean;
  onClose: () => void;
  onImport: (animals: Array<{ id: string, lote: string, ubicacion: string, peso: string }>) => void;
}

// Simulación simple de parseo Excel (.csv/.txt/.xlsx no soportado nativamente en frontend)
// Solo para demo: espera columnas id,lote,ubicacion,peso en CSV simple sin encabezado
function parseCSV(text: string) {
  ...
  }).filter(animal => animal.id && animal.lote && animal.ubicacion && animal.peso);
}
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files?.[0]) {
      ...
      reader.onload = (evt) => {
        ...
        if (parsed.length === 0) {
          setError("No se encontraron datos válidos (debe ser CSV: id,lote,ubicacion,peso)");
          return;
        }
        setAnimals(parsed);
        setError(null);
  ...
  return (
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent side="bottom" className="h-[70vh]">
        <SheetHeader>
          <SheetTitle className="text-[#3a210c]">Importar animales desde Excel/CSV</SheetTitle>
        </SheetHeader>
        <div className="p-4 space-y-4">
          {step === "idle" && (
            <>
              <Label className="block">Selecciona un archivo (CSV, formato: <code>id,lote,ubicacion,peso</code> por línea):</Label>
          ...
          {step === "preview" && (
            <>
              <div className="mb-2 text-sm text-gray-500">Preview de animales a importar ({animals.length}):</div>
